...
namespace ErikEngine {
    void ParseGo(Board& board, SearchInfo& search_info, std::vector<std::string>& args) {
        ...
        if (timeSet) {
            time /= movestogo;
            time += inc;
            time = std::max(0, time - 50); // safety

            search_info.startTime = Utils::GetTimeMS();
            search_info.stopTime = search_info.startTime + static_cast<U64>(time);
        }
    ...
    void UCILoop(Board& board, Searcher& searcher, SearchInfo& search_info) {
        std::cout << "uciok\n";
